...
Shader "Hidden/EdgeDetect" { 
	...
	v2flum vertLum (appdata_img v)
	{
		...
		#if UNITY_5_4_1
			o.uv[0] = uv, _MainTex_ST);
			o.uv[1] = uv + float2(-_MainTex_TexelSize.x, -_MainTex_TexelSize.y) * _SampleDistance, _MainTex_ST);
			o.uv[2] = uv + float2(+_MainTex_TexelSize.x, -_MainTex_TexelSize.y) * _SampleDistance, _MainTex_ST);
		#else
			o.uv[0] = uv;
	...
	v2f vertRobert( appdata_img v ) 
	{
		...
		#if UNITY_5_4_1
			o.uv[1] = uv + _MainTex_TexelSize.xy * half2(1,1) * _SampleDistance, _MainTex_ST);
			o.uv[2] = uv + _MainTex_TexelSize.xy * half2(-1,-1) * _SampleDistance, _MainTex_ST);
			o.uv[3] = uv + _MainTex_TexelSize.xy * half2(-1,1) * _SampleDistance, _MainTex_ST);
			o.uv[4] = uv + _MainTex_TexelSize.xy * half2(1,-1) * _SampleDistance, _MainTex_ST);
		#else
...
#if UNITY_5_4_1
		o.uv[1] = uv, _MainTex_ST);
		o.uv[4] = uv, _MainTex_ST);
				
		// offsets for two additional samples
		o.uv[2] = uv + float2(-_MainTex_TexelSize.x, -_MainTex_TexelSize.y) * _SampleDistance, _MainTex_ST);
		o.uv[3] = uv + float2(+_MainTex_TexelSize.x, -_MainTex_TexelSize.y) * _SampleDistance, _MainTex_ST);
#else
		o.uv[1] = uv;
		o.uv[4] = uv;
